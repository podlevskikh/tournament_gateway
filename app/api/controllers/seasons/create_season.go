...
func NewCreateSeason(service Service, logger *zerolog.Logger) *CreateSeason {
	return &CreateSeason{service: service, logger: logger}
}

func (s *CreateSeason) HTTPHandler(c *gin.Context) {
	...
	seas, err := s.service.CreateSeason(c.Request.Context(), se)
	if err != nil {
